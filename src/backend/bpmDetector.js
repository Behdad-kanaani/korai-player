...
async function detectRealBPM(filePath) {
    console.log(`🔍 Detecting real BPM for: ${path.basename(filePath)}`);
    ...
    try {
        ...
        if (metadata.common.bpm && metadata.common.bpm > 0) {
            console.log(`✅ BPM from metadata: ${metadata.common.bpm}`);
    ...
    if (!audioData) {
        console.log('⚠️ Could not read audio, using fallback');
        return 120;
    }
    ...
    console.log(`📊 Detection results: Peaks=${bpmPeaks}, Auto=${bpmAuto}, Onset=${bpmOnset}`);
