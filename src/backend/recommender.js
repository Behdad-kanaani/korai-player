 ...
 * - Multi-dimensional similarity scoring (BPM, Energy, Genre, Spectral features)
...
function detectGenre(track) {
    ...
    // Jazz / Blues / R&B (low BPM, low-medium energy)
    if (bpm < 95) {
    ...
    // Hip Hop / Rap (low-medium BPM, medium-high energy)
    if (bpm >= 70 && bpm < 100) {
    ...
    // Pop (medium BPM, medium-high energy)
    if (bpm >= 95 && bpm < 125) {
    ...
    // Rock / Alternative (medium-high BPM, high energy)
    if (bpm >= 100 && bpm < 150) {
...
/**
 * Calculate similarity score between two tracks using weighted multi-dimensional metrics
 */
function calculateSimilarityScore(sourceTrack, targetTrack) {
    ...
    let bpmScore = 0;
    if (bpmDiff === 0) bpmScore = 35;
    else if (bpmDiff <= 3) bpmScore = 33;
    else if (bpmDiff <= 5) bpmScore = 30;
    else if (bpmDiff <= 8) bpmScore = 26;
    else if (bpmDiff <= 12) bpmScore = 22;
    else if (bpmDiff <= 18) bpmScore = 17;
    else if (bpmDiff <= 25) bpmScore = 12;
    else if (bpmDiff <= 35) bpmScore = 7;
    ...
    let energyScore = 0;
    if (energyDiff === 0) energyScore = 25;
    else if (energyDiff <= 0.03) energyScore = 23;
    else if (energyDiff <= 0.06) energyScore = 20;
    else if (energyDiff <= 0.1) energyScore = 17;
    else if (energyDiff <= 0.15) energyScore = 13;
    else if (energyDiff <= 0.22) energyScore = 9;
    else if (energyDiff <= 0.3) energyScore = 5;
    else energyScore = 0;
    
    // Genre similarity score (using detected genre)
    ...
    if (sourceGenre.name === targetGenre.name) {
        genreScore = 40;
        similarityReason = `${sourceGenre.name} (سبک یکسان)`;
        similarityIcon = '🎯';
    } else {
        ...
        if (sourceFamily.includes(targetGenre.name)) {
            genreScore = 25;
            similarityReason = `${targetGenre.name} (سبک مرتبط)`;
            similarityIcon = '🎧';
        } else if (bpmDiff < 10) {
            genreScore = 20;
            similarityReason = 'BPM بسیار نزدیک';
            similarityIcon = '⚡';
        } else if (energyDiff < 0.08) {
            genreScore = 15;
            similarityReason = 'انرژی صوتی مشابه';
            similarityIcon = '🔊';
        } else {
            similarityReason = 'توصیه هوشمند';
            similarityIcon = '🤖';
        }
    }
    
    // Popularity bonus (from play count and like count - max 10 points)
    const playCount = targetTrack.playCount || 0;
    const likeCount = targetTrack.likeCount || 0;
    let popularityBonus = Math.min(8, (playCount * 0.05) + (likeCount * 0.8));
    
    // Total similarity score (max 100)
    let totalSimilarity = Math.min(98, Math.floor(bpmScore + energyScore + genreScore + popularityBonus));
...
function getRecommendations(allTracks, sourceTrack, limit = 12) {
    ...
    const scoredTracks = otherTracks.map(track => {
        ...
        if (similarity.score < 15) return null;
...
function getDiverseRecommendations(allTracks, sourceTrack, limit = 15) {
    ...
    const sorted = validScores.sort((a, b) => {
        // If both are same genre as source, sort by similarity
        if (a.isDifferentGenre === b.isDifferentGenre) {
            return b.similarity - a.similarity;
        }
        // Prefer same genre for top positions, but include diverse ones in lower positions
